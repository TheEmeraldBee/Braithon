...
class LoopModule(Module):
    ...
    def handle_command(self, interpreter: Interpreter):
        if interpreter.context.command() == "[":
            ...
            past_program.program_string_cursor = (
        ...
        if interpreter.context.command_keyword("break("):
            ...
            if interpreter.context.command() != ")":
                return Result().with_error(
                    Error(-1, f"Expected ) but found '{interpreter.context.command()}'")
                )
        ...
        if interpreter.context.command_keyword("break"):
            interpreter.context.program_string_cursor = len(
                interpreter.context.program_string
            )
            return Result()
